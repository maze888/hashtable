...
static int ht_insert_inter(HashTable *ht, void *key, size_t key_len, void *value, size_t value_len)
{
	...
	if ( bucket ) { // insert to bucket link
		...
		if ( !(bucket_next = malloc(sizeof(HashTableBucket))) ) {
			HT_SET_ERROR("malloc() is failed: (errmsg: %s, errno: %d, size: %lu)", strerror(errno), errno, sizeof(HashTableBucket));
			goto out;
		}
		...
		// extend hashtable buckets (TODO: multikey)
...
static int ht_insert(HashTable *ht, const void *key, size_t key_len, const void *value, size_t value_len)
{
	...
	if ( !ht || !key || key_len == 0 || !value || value_len == 0 ) {
		HT_SET_ERROR("invalid argument: (ht = %s, key = %s, key_len = %lu, value = %s, value_len = %lu)", CKNUL(ht), CKNUL(key), key_len, CKNUL(value), value_len);
		return -1;
	}
	
	key_copy = calloc(1, key_len + 1);
	if ( !key_copy ) {
		HT_SET_ERROR("calloc() is failed: (errmsg: %s, errno: %d, size: %lu)", strerror(errno), errno, key_len + 1);
		goto out;
	}
	value_copy = calloc(1, value_len + 1);
	if ( !value_copy ) {
		HT_SET_ERROR("calloc() is failed: (errmsg: %s, errno: %d, size: %lu)", strerror(errno), errno, value_len + 1);
		goto out;
	}
